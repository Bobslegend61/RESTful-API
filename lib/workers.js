...
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
    ...
    workers.log(originalCheckData, checkOutcome, state, alertWanrranted, timeOfCheck);
...
workers.alertUserToStatusChanged = (newCheckData) => {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, err => {
        if(!err) {
            debug('Success: User was alerted to a status change in the check via sms:', msg);
        }else {
...
workers.init = () => {
    ...
    console.log('\x1b[33m%s\x1b[0m', 'Background of workers are running');
